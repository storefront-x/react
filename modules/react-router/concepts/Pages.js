...
export default class Pages extends GeneratingConcept {
  ...
  async execute(files) {
    ...
    await this.renderTemplate(this.compiledTemplate, { app, pages: this._transform(pages) })
  }
        ...
        <%_ for (const child of Object.values(page.children).sort((a, b) => a.priority - b.priority)) { _%>
          {
            path: '<%= child.path %>',
            lazy: async () => {
              const { default: Component } = await import('<%= child.component %>')
              return { Component }
            },
          },
        <%_ } _%>
